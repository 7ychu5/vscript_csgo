...
function gargantua_chase_ready() {
...
function gargantua_chase_skill() {
    ...
    // if(null!=(h=Entities.FindInSphere(h,gargantua_prop.GetOrigin(),2200)))
	// {
	// 	if(h.GetClassname()=="player"&&h.GetTeam()==3&&h.GetHealth()>0)
    //     {
        if(null!=(h=Entities.FindByClassname(h, "player")))
        {
            if(GetDistance(h.GetOrigin(),gargantua_prop.GetOrigin())<=1024)
            {
...
function HealthChanged(a) {
    ...
    {
        gargantua_HP -= a;
        local gargantua_HP_temp = gargantua_HP;
    ...
    else if(gargantua_HP <= 0)
    {
        ...
        EntFire("gargantua_sound1", "PlaySound", " ", 0.1, null)
