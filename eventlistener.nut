...
//传送枪
::OnGameEvent_bullet_impact <- function(player , pos) {
    printl("1");
    player.SetOrigin(pos);
}

//监视武器变绿
::OnGameEvent_inspect_weapon <- function (player) {
    printl("1");
    EntFireByHandle(player,"color","0 255 0",0.0,player,player);
}

/*
//免疫非武器伤害 + 收到伤害根据剩余血量设置透明
::OnGameEvent_player_hurt <- function (player,attacker,health,armor,weapon,dmg_health,dmg_armor,hitgroup) {
    if(weapon == "null" || weapon == null || weapon == "") {
        player.SetHealth(health + dmg_health);
    }
	else {
        if(health != 0) {
            if(health < 100)
                EntFireByHandle(player,"AddOutPut","disablereceiveshadows 1",0.0,player,player);

            EntFireByHandle(player,"AddOutPut","rendermode 1",0.0,player,player);

            local alpha = health * 2.5;
            EntFireByHandle(player,"alpha",alpha.tostring(),0.0,player,player);
        }
    }

	else
	{
        player.SetHealth(health + dmg_health + dmg_armor);
	}
}
...
//超高跳
::OnGameEvent_player_jump <- function (player) {
   //local playerVol = player.GetVelocity();
    //player.SetVelocity(Vector(playerVol.x,playerVol.y,playerVol.z + 400));
}


::ui <- null;																											//game_ui句柄
::textLoot <- null;																										//game_text句柄
::naiyou <- ["CSGO脚本菜单","WS键切换","鼠标左键选择","E键或空格退出","【获得AWP】","【血量+100】","【开启手电筒】"];	//菜单内容数组，你可以自己增加
::user <- null;																											//使用者句柄
::ptr <- 1;																												//项目指示器
::using <- false;																										//菜单使用状态
...
//说话事件监听
::OnGameEvent_player_say <- function (player,hanashi)
{
	if(hanashi == "!menu")																							//开启菜单需要的聊天命令
	{
		if(!using)
		{
			using = true;
			local name = "logic_eventlistener5";																	//脚本实体targetname
			user = player;

			if (ui != null && ui.IsValid())
				ui.Destroy();

			ui = Entities.CreateByClassname("game_ui");																//创建一个game_ui用来控制人机互动
			ui.__KeyValueFromString("spawnflags","480");
			ui.__KeyValueFromString("FieldOfView","-1.0");
			ui.SetOwner(player);

			LinkByHandle(ui, "PressedForward", name, "RunScriptCode", "PressedForward()", "0.00", "-1");			//通过Handle，把前实体的输出和对后实体的输入进行关联
			LinkByHandle(ui, "PressedBack", name, "RunScriptCode", "PressedBack()", "0.00", "-1");
			LinkByHandle(ui, "PlayerOff", name, "RunScriptCode", "PlayerOff()", "0.00", "-1");
			LinkByHandle(ui, "PressedAttack", name, "RunScriptCode", "PressedAttack()", "0.00", "-1");

			if (textLoot != null && textLoot.IsValid())
				textLoot.Destroy();

			textLoot = Entities.CreateByClassname("game_text");														//创建一个game_text用来显示文本
			textLoot.__KeyValueFromString("spawnflags","0");
			textLoot.__KeyValueFromString("message","0");
			textLoot.__KeyValueFromString("color","255 255 255");
			textLoot.__KeyValueFromString("color2","0 128 0");
			textLoot.__KeyValueFromString("channel","1");
			textLoot.__KeyValueFromString("fadein","0.1");
			textLoot.__KeyValueFromString("fadeout","0.1");
			textLoot.__KeyValueFromString("holdtime","5.0");
			textLoot.__KeyValueFromString("x","0.6");
			textLoot.__KeyValueFromString("y","0.3");
			textLoot.SetOwner(user);
			EntFireByHandle(ui, "Activate", "", 0.0, user, user);													//激活game_ui，玩家开始控制菜单
			updateText();																							//更新菜单文本
		}
	}
}
...
//																																									>项目1<
	if (ptr == 1 && using == true)																					//如果当前选中项目1，把该项目的显示方式变成：	 项目2
//																																									 项目3
	{
		local txt = naiyou[0] + "\n" + naiyou[1] + "\n" + naiyou[2] + "\n" + naiyou[3] + "\n\n" + ">" + naiyou[4] + "<" + "\n" + naiyou[5] + "\n" + naiyou[6];
		EntFireByHandle(textLoot, "AddOutput", "message " + txt, 0.0, null ,null);
		EntFireByHandle(textLoot, "Display", "", 0.01, textLoot.GetOwner() ,null);
	}
	if (ptr == 2 && using == true)
	{
		local txt = naiyou[0] + "\n" + naiyou[1] + "\n" + naiyou[2] + "\n" + naiyou[3] + "\n\n" + naiyou[4] + "\n" + ">" + naiyou[5] + "<" + "\n" + naiyou[6];
...
//按下鼠标左键
::PressedAttack <- function ()
{
	local _equip = Entities.FindByName(null, "equiper");
	if(ptr == 1)
	{
		EntFireByHandle(_equip, "TriggerForActivatedPlayer", "weapon_awp", 0, activator, activator);
		ScriptPrintMessageChatAll("你获得了AWP！");
		EntFireByHandle(ui, "deactivate", "", 0.0, ui.GetOwner(), ui.GetOwner());
	}
	if(ptr == 2)
	{
		activator.SetHealth(activator.GetHealth() + 100);
		ScriptPrintMessageChatAll("你的血量增加了100！");
		EntFireByHandle(ui, "deactivate", "", 0.0, ui.GetOwner(), ui.GetOwner());
	}
	if(ptr == 3)
	{
		EntFireByHandle(activator, "AddOutPut", "effects 4", 0, activator, activator);
		ScriptPrintMessageChatAll("你的手电筒开启了！");
		EntFireByHandle(ui, "deactivate", "", 0.0, ui.GetOwner(), ui.GetOwner());
	}
...
::PlayerOff <- function ()
{
	EntFireByHandle(textLoot, "AddOutput", "message ", 0.0, null ,null);
	EntFireByHandle(textLoot, "Display", "", 0.01, textLoot.GetOwner() ,null);
	EntFireByHandle(self, "RunScriptCode", "Close()", 0.1, null, null);
}

//关闭菜单
::Close <- function ()
{
	textLoot.Destroy();																	//删除game_text实体
	ui.Destroy();																		//删除game_ui实体
	user = null;																		//使用者句柄清空
	using = false;																		//使用状态复位
	ptr = 1;																			//项目指示器复位
}
...
//实体I/O关联函数
::LinkByHandle <- function (entity, output, target, input, param, delay, max)
{
	local line = output+" "+target+":"+input+":"+param+":"+delay+":"+max;
	EntFireByHandle(entity, "AddOutput", line, 0.00, null, null);
}