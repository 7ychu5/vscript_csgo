...
function detect_user() {
    if(activator.GetTeam()==2)
    {
        ...
        EntFire("minigun_ui","Deactivate","",0.0,null);
...
function minigun_display() {
    local text;
    if(null == Entities.FindByName(null, "game_text_minigun"))
    {
        text = Entities.CreateByClassname("game_text");
        text.__KeyValueFromString("targetname", "game_text_minigun")
        text.__KeyValueFromString("message","game_text");
        text.__KeyValueFromString("color","164 43 36");
    ...
    else text = Entities.FindByName(null, "game_text_minigun");
    ...
    local minigun_user = Entities.FindByName(null, "minigun_user");
    EntFireByHandle(text, "Display", "", 0.0, minigun_user, null);
}
...
function tick_add(tick_toggle) {
    ...
    {
        if (sec<6) {
            EntFire("minigun_gunfire_push", "AddOutput", "speed 200", 0.0, null);
            EntFire("minigun_gunfire_push", "enable", "", 0.0, activator);
            EntFire("minigun_gunfire_push", "disable", "", 0.2, activator);
        }
        else if(sec<11){
            EntFire("minigun_gunfire_push", "AddOutput", "speed 250", 0.0, null);
            EntFire("minigun_gunfire_push", "enable", "", 0.0, activator);
            EntFire("minigun_gunfire_push", "disable", "", 0.4, activator);
        }
        else if(sec<sec_max){
            EntFire("minigun_gunfire_push", "AddOutput", "speed 350", 0.0, null);
            EntFire("minigun_gunfire_push", "enable", "", 0.0, activator);
            EntFire("minigun_gunfire_push", "disable", "", 0.6, activator);
        }
        else{
            ...
            EntFire("minigun_destroy","trigger","",1.0,null);//是否摧毁minigun本体
